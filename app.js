 ...
 * ============================================================================
 * Pure client-side simulation, interactive canvas with pan/zoom,
 * analytical combinatorial metrics, and high-res PNG/SVG export.
 */
...
(() => {
  ...
  function updateAnalytics() {
    ...
    if (isTriangle) {
      ...
      // Newly removed in step n: 3^(n-1) if n >= 1, else 0
      const stepRemoved = n === 0 ? 0n : BigPow(3, n - 1);
      // Cumulative removed: (3^n - 1) / 2
      ...
      // 3. Removed Shapes Count
      // Newly removed in step n: 8^(n-1) if n >= 1, else 0
      const stepRemoved = n === 0 ? 0n : BigPow(8, n - 1);
      // Cumulative removed: (8^n - 1) / 7
   ...
   */
  function renderSierpinskiTriangle(targetCtx, scale, offX, offY, viewWidth, viewHeight) {
    const n = state.depth;
    const s = BASE_SIZE;
    ...
    if (state.mode === 'cutouts') {
      // Cutouts mode: Draw big base triangle, then highlight excavated inverted triangles
      ...
      if (n > 0) {
        ...
        targetCtx.beginPath();
        drawTriangleHoles(targetCtx, p1, p2, p3, n);
        targetCtx.fill();
      }
      return;
    }
    ...
    targetCtx.beginPath();
    generateTriangles(targetCtx, p1, p2, p3, n, scale);
  ...
  function generateTriangles(tCtx, a, b, c, depth, scale) {
    if (depth === 0) {
      tCtx.moveTo(a.x, a.y);
      tCtx.lineTo(b.x, b.y);
      tCtx.lineTo(c.x, c.y);
    ...
    generateTriangles(tCtx, a, ab, ca, depth - 1, scale);
    generateTriangles(tCtx, ab, b, bc, depth - 1, scale);
    generateTriangles(tCtx, ca, bc, c, depth - 1, scale);
  }

  function drawTriangleHoles(tCtx, a, b, c, depth) {
    if (depth === 0) return;
    ...
    drawTriangleHoles(tCtx, a, ab, ca, depth - 1);
    drawTriangleHoles(tCtx, ab, b, bc, depth - 1);
    drawTriangleHoles(tCtx, ca, bc, c, depth - 1);
  }
   ...
   * - Viewport Frustum Culling + Screen Pixel LOD avoids recursing sub-pixels (< 0.35px)
   */
  function renderSierpinskiCarpet(targetCtx, scale, offX, offY, viewWidth, viewHeight) {
    const n = state.depth;
    const s = BASE_SIZE;
    const halfS = s / 2;
    ...
    if (state.mode === 'cutouts') {
      // Cutouts mode: background base, then highlight all excavated voids
      targetCtx.fillStyle = state.backgroundColor;
      targetCtx.fillRect(-halfS, -halfS, s, s);

      if (n > 0) {
        targetCtx.fillStyle = state.patternColor;
        drawCarpetCutouts(targetCtx, -halfS, -halfS, s, n, scale, minX, maxX, minY, maxY);
      }
      return;
    }

    if (state.mode === 'wireframe') {
      // Wireframe mode: stroke the perimeter and recursive cuts
      targetCtx.strokeStyle = state.patternColor;
      targetCtx.lineWidth = Math.max(1 / scale, 1.2);
      targetCtx.strokeRect(-halfS, -halfS, s, s);

      if (n > 0) {
        drawCarpetWireframe(targetCtx, -halfS, -halfS, s, n, scale, minX, maxX, minY, maxY);
      }
      return;
    }
    ...
    targetCtx.fillStyle = state.backgroundColor;
    drawCarpetHoles(targetCtx, -halfS, -halfS, s, n, scale, minX, maxX, minY, maxY);
  }

  function drawCarpetHoles(tCtx, x, y, size, depth, scale, minX, maxX, minY, maxY) {
    // Frustum cull
    ...
    if (depth > 1) {
      ...
      drawCarpetHoles(tCtx, x,           y,           sub, nextDepth, scale, minX, maxX, minY, maxY);
      drawCarpetHoles(tCtx, x + sub,     y,           sub, nextDepth, scale, minX, maxX, minY, maxY);
      drawCarpetHoles(tCtx, x + 2 * sub, y,           sub, nextDepth, scale, minX, maxX, minY, maxY);
      drawCarpetHoles(tCtx, x,           y + sub,     sub, nextDepth, scale, minX, maxX, minY, maxY);
      drawCarpetHoles(tCtx, x + 2 * sub, y + sub,     sub, nextDepth, scale, minX, maxX, minY, maxY);
      drawCarpetHoles(tCtx, x,           y + 2 * sub, sub, nextDepth, scale, minX, maxX, minY, maxY);
      drawCarpetHoles(tCtx, x + sub,     y + 2 * sub, sub, nextDepth, scale, minX, maxX, minY, maxY);
      drawCarpetHoles(tCtx, x + 2 * sub, y + 2 * sub, sub, nextDepth, scale, minX, maxX, minY, maxY);
    }
  }

  function drawCarpetWireframe(tCtx, x, y, size, depth, scale, minX, maxX, minY, maxY) {
    if (x > maxX || x + size < minX || y > maxY || y + size < minY) return;
    if (size * scale < 1.0) return;
    ...
    if (depth > 1) {
      const nextDepth = depth - 1;
      drawCarpetWireframe(tCtx, x,           y,           sub, nextDepth, scale, minX, maxX, minY, maxY);
      drawCarpetWireframe(tCtx, x + sub,     y,           sub, nextDepth, scale, minX, maxX, minY, maxY);
      drawCarpetWireframe(tCtx, x + 2 * sub, y,           sub, nextDepth, scale, minX, maxX, minY, maxY);
      drawCarpetWireframe(tCtx, x,           y + sub,     sub, nextDepth, scale, minX, maxX, minY, maxY);
      drawCarpetWireframe(tCtx, x + 2 * sub, y + sub,     sub, nextDepth, scale, minX, maxX, minY, maxY);
      drawCarpetWireframe(tCtx, x,           y + 2 * sub, sub, nextDepth, scale, minX, maxX, minY, maxY);
      drawCarpetWireframe(tCtx, x + sub,     y + 2 * sub, sub, nextDepth, scale, minX, maxX, minY, maxY);
      drawCarpetWireframe(tCtx, x + 2 * sub, y + 2 * sub, sub, nextDepth, scale, minX, maxX, minY, maxY);
    }
  }

  function drawCarpetCutouts(tCtx, x, y, size, depth, scale, minX, maxX, minY, maxY) {
    if (x > maxX || x + size < minX || y > maxY || y + size < minY) return;
    if (size * scale < 0.35) return;
    ...
    if (depth > 1) {
      const nextDepth = depth - 1;
      drawCarpetCutouts(tCtx, x,           y,           sub, nextDepth, scale, minX, maxX, minY, maxY);
      drawCarpetCutouts(tCtx, x + sub,     y,           sub, nextDepth, scale, minX, maxX, minY, maxY);
      drawCarpetCutouts(tCtx, x + 2 * sub, y,           sub, nextDepth, scale, minX, maxX, minY, maxY);
      drawCarpetCutouts(tCtx, x,           y + sub,     sub, nextDepth, scale, minX, maxX, minY, maxY);
      drawCarpetCutouts(tCtx, x + 2 * sub, y + sub,     sub, nextDepth, scale, minX, maxX, minY, maxY);
      drawCarpetCutouts(tCtx, x,           y + 2 * sub, sub, nextDepth, scale, minX, maxX, minY, maxY);
      drawCarpetCutouts(tCtx, x + sub,     y + 2 * sub, sub, nextDepth, scale, minX, maxX, minY, maxY);
      drawCarpetCutouts(tCtx, x + 2 * sub, y + 2 * sub, sub, nextDepth, scale, minX, maxX, minY, maxY);
    }
  }
  ...
  function render() {
    ...
    if (state.type === 'triangle') {
      renderSierpinskiTriangle(ctx, state.camera.zoom, state.camera.x, state.camera.y, cssW, cssH);
    } else {
      renderSierpinskiCarpet(ctx, state.camera.zoom, state.camera.x, state.camera.y, cssW, cssH);
    }
  ...
  dom.canvasViewport.addEventListener('pointerup', endDrag);
  dom.canvasViewport.addEventListener('pointercancel', endDrag);
  ...
  function exportPNG() {
    ...
    if (state.type === 'triangle') {
      renderSierpinskiTriangle(expCtx, exportScale, 0, 0, size, size);
    } else {
      renderSierpinskiCarpet(expCtx, exportScale, 0, 0, size, size);
    }
    expCtx.restore();
